...
_WA_TS = re.compile(r"(\d{1,2}\.\d{1,2}\.\d{2,4},\s*\d{1,2}:\d{2}\s*-\s*)")


def chunk_text(text: str, max_chars: int = MAX_SEG_CHARS):
    """Teilt Text an Zeilen-/Absatzgrenzen in Segmente von ~max_chars Laenge.
    WhatsApp-Exporte ohne Zeilenumbrueche: vor jedem Zeitstempel umbrechen, damit
    Nachrichten nicht mitten im Satz zerschnitten werden."""
    if text.count("\n") < 5 and len(_WA_TS.findall(text)) > 5:
        text = _WA_TS.sub(r"\n\1", text)
